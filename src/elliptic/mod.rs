//!
//!
//!

use rand::Rng;

use crate::{
    arith::{Arith, CoreArith},
    UInt,
};

const BYTES_10K_LEN: usize = 1806;

/// Bytes for lcm(1,...,B), where the upper bound B equals 10_000.
///
/// To reproduce the following array:
/// 1) Compute lcm(1,...,B) for B = 10_000, and convert the result to base 2 (binary)
/// 2) Decompose the base 2 representation to an array of bytes (each element has 8 bits)
///
/// E.g., in the following array hexadecimal 0x9b contains the first 8 bits of the base 2 representation,
/// 0x2c the next 8 bits etc.
static BYTES_10K: [u8; BYTES_10K_LEN] = [
    ,
];

pub struct EllipticCurve<T: UInt> {
    x: T,
    y: T,
}

impl<T: UInt> EllipticCurve<T> {
    ///
    pub fn compute_maybe_factor_from_curve(num: T) -> T {
        let mut curve = EllipticCurve {
            x: T::one(),
            y: T::one(),
        };

        match curve.init_rnd_point(num) {
            (true, a) => T::gcd_mod(curve.montgomery_ladder(a, num), num),
            (false, a) => a,
        }
    }

    /// Suyama's parametrization
    ///
    fn init_rnd_point(&mut self, num: T) -> (bool, T) {
        // u8::MAX or u32::MAX
        let sigma = rand::thread_rng().gen_range(6..u8::MAX).into();

        let u = T::sub_mod(T::mult_mod(sigma, sigma, num), 5.into(), num);
        let u3 = T::exp_mod_unsafe(u, 3.into(), num);
        let v = T::mult_mod(sigma, 4.into(), num);

        self.x = u3;
        self.y = T::exp_mod_unsafe(v, 3.into(), num);

        let vu_diff = T::exp_mod_unsafe(T::sub_mod(v, u, num), 3.into(), num);
        let uv_add = T::add_mod_unsafe(T::mult_mod_unsafe(u, 3.into(), num), v, num);

        let a_numer = T::mult_mod_unsafe(vu_diff, uv_add, num);
        let a_denumer = T::mult_mod_unsafe(T::mult_mod(u3, 4.into(), num), v, num);
        let a_denumer_inv = T::multip_inv(a_denumer, num);

        if a_denumer_inv == T::zero() {
            // no inverse for `a_denumer`
            return (false, T::gcd_mod(a_denumer, num));
        }

        let mut a = T::sub_mod_unsafe(
            T::mult_mod_unsafe(a_numer, a_denumer_inv, num),
            2.into(),
            num,
        );

        a = T::mult_mod_unsafe(
            T::add_mod_unsafe(a, 2.into(), num),
            T::multip_inv(4.into(), num),
            num,
        );

        (true, a)
    }

    ///
    ///
    fn elliptic_double(&mut self, a: T, num: T) {
        let psum = T::add_mod(self.x, self.y, num);
        let psub = T::sub_mod(self.x, self.y, num);

        let psum_square = T::mult_mod_unsafe(psum, psum, num);
        let psub_square = T::mult_mod_unsafe(psub, psub, num);

        let pmix = T::sub_mod_unsafe(psum_square, psub_square, num);

        self.x = T::mult_mod_unsafe(psum_square, psub_square, num);

        self.y = T::mult_mod_unsafe(
            pmix,
            T::add_mod_unsafe(psub_square, T::mult_mod_unsafe(a, pmix, num), num),
            num,
        );
    }

    ///
    ///
    fn elliptic_add(&mut self, point: &Self, point0: &Self, num: T) {
        let lp_sum = T::add_mod(self.x, self.y, num);
        let lp_sub = T::sub_mod(self.x, self.y, num);

        let rp_sum = T::add_mod(point.x, point.y, num);
        let rp_sub = T::sub_mod(point.x, point.y, num);

        let lterm = T::mult_mod_unsafe(lp_sub, rp_sum, num);
        let rterm = T::mult_mod_unsafe(lp_sum, rp_sub, num);

        let term_add = T::add_mod_unsafe(lterm, rterm, num);
        let term_sub = T::sub_mod_unsafe(lterm, rterm, num);

        self.x = T::mult_mod_unsafe(point0.y, T::mult_mod_unsafe(term_add, term_add, num), num);

        self.y = T::mult_mod_unsafe(point0.x, T::mult_mod_unsafe(term_sub, term_sub, num), num);
    }

    ///
    ///
    fn montgomery_ladder(&self, a: T, num: T) -> T {
        let mut q = EllipticCurve {
            x: self.x,
            y: self.y,
        };
        let mut p = EllipticCurve {
            x: self.x,
            y: self.y,
        };

        p.elliptic_double(a, num);

        let bits = u8::BITS;
        let ms_bit_idx = u8::BITS - 1;

        let last_byte_idx = BYTES_10K_LEN - 1;

        for (byte_idx, byte_val) in BYTES_10K.iter().enumerate() {
            for cbit in (0..bits).rev() {
                if (byte_idx == 0 && cbit == ms_bit_idx) || (byte_idx == last_byte_idx && cbit == 0)
                {
                    continue;
                }

                if (*byte_val >> cbit) & 1 == 1 {
                    q.elliptic_add(&p, self, num);
                    p.elliptic_double(a, num);
                } else {
                    p.elliptic_add(&q, self, num);
                    q.elliptic_double(a, num);
                }
            }
        }

        q.y
    }
}

#[cfg(test)]
mod tests;
